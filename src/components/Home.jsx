...
const filmes1 = [
    ...
    },
    {
        ...
        descricao: 'Um jovem casal viaja para uma ilha remota para comer em um restaurante exclusivo onde o chef preparou um menu suntuoso, com algumas surpresas chocantes.',
        idclassificacao: 6
    },
    {
        idfilme: 9,
        titulo: 'John Wick: Chapter 4',
        poster: 'https://resizing.flixster.com/tpLtbDZQSwBpb3jfwoI7zMiRMjE=/206x305/v2/https://resizing.flixster.com/ayiSi8HrUS3_i-Jkru1P2oMM58Q=/ems.cHJkLWVtcy1hc3NldHMvbW92aWVzLzhiODAwYWIyLTM1MmItNGI1ZS1iOWQzLTdjZjAzMzI1MDc2Yy5qcGc=',
        descricao: 'John Wick enfrenta novos inimigos e forma alianças inesperadas enquanto tenta destruir a Alta Cúpula, a organização criminosa global.',
        idclassificacao: 6
    },
    {
        idfilme: 10,
        titulo: 'A Pequena Sereia',
        poster: 'https://resizing.flixster.com/xYr4IyGjYuN05tWqASKzUlqsEvg=/206x305/v2/https://resizing.flixster.com/-XZAfHZM39UwaGJIFWKAE8fS0ak=/v3/t/assets/p11958_p_v13_ab.jpg',
        descricao: 'Ariel, a pequena sereia, faz um pacto com a bruxa do mar Úrsula para se tornar humana e conquistar o amor do príncipe Eric.',
        idclassificacao: 3
    },
    {
        idfilme: 11,
...
const filmes = [
    ...
    },
    {
        ...
        escritores: 'Shay Hatten, Michael Finch',
        estrelas: 'Keanu Reeves, Laurence Fishburne',
        disponibilidade: true,
    ...
    },
    {
        ...
        estrelas: 'Halle Bailey, Jonah Hauer-King',
        disponibilidade: true,
        cidade: 'Recife',
        estado: 'PE',
        idadeMin: 10,
...
const generos = ['Ação', 'Comédia', 'Drama', 'Fantasia', 'Terror','Ficção Científica', 'Animação'];

const idadesMin = [6, 10, 12, 14, 16, 18];

const disponibilidades = [true, false];

export function Home (){

    const [button_list, set_list] = useState (['src/assets/ticket.svg','src/assets/ticket2.svg','src/assets/ticket3.svg']);
    ...
    const [idadeMinSelecionada, setIdadeMinSelecionada] = useState('');

    const handleIdadeMinChange = (event) => {
        setIdadeMinSelecionada(event.target.value);
        setCurrentPage(1);
    };

    const [disponibilidadeSelecionada, setDisponibilidadeSelecionada] = useState('');

    const handleDisponibilidadeChange = (event) => {
        setDisponibilidadeSelecionada(event.target.value);
        setCurrentPage(1);
    };

    const filmesFiltrados = filmes.filter(filme => 
        (!tituloSelecionado || filme.titulo.toLowerCase().includes(tituloSelecionado.toLowerCase())) &&
        (!estadoSelecionado || filme.estado === estadoSelecionado) &&
        (!cidadeSelecionada || filme.cidade === cidadeSelecionada) &&
        (!generoSelecionado || filme.genero === generoSelecionado) &&
        (!disponibilidadeSelecionada || filme.disponibilidade === (disponibilidadeSelecionada === 'true')) &&
        (!idadeMinSelecionada || filme.idadeMin <= idadeMinSelecionada)
    );
    ...
    const currentFilms = filmesFiltrados.slice(indexOfFirstFilm, indexOfLastFilm);

    const paginate = (pageNumber) => setCurrentPage(pageNumber);

    const pageNumbers = [];

    for (let i = 1; i <= Math.ceil(filmesFiltrados.length / filmsPerPage); i++) {
        pageNumbers.push(i);
    }
    ...
    const handleSelectButtonClick = (idMovie) => {
        navigate('/MovieSession', { state: { idMovie: idMovie } });
    };
    ...
    return(
        <div className={styles.all}>
            ...
            <div className={styles.movie_list}>
                <div className={styles.movie_selection}>
                    <div className={styles.movie_selection_name}>
                            <input id="titulo" onChange={handleTituloChange} placeholder='Digite o nome do Filme'></input>
                    ...
                    <div>
                        <select id="genero" value={generoSelecionado} onChange={handleGeneroChange}>
                    ...
                    <div>
                        <select id="idadeMin" value={idadeMinSelecionada} onChange={handleIdadeMinChange}>
                            <option value="">Faixa Etária</option>
                            {idadesMin.map(idadeMin => (
                                <option key={idadeMin} value={idadeMin}>{idadeMin}</option>
                            ))}
                        </select>
                    </div>
                    <div>
                        <select id="disponibilidade" value={disponibilidadeSelecionada} onChange={handleDisponibilidadeChange}>
                            <option value="">Disponibilidade</option>
                            {disponibilidades.map(disponibilidade => (
                                <option key={disponibilidade} value={disponibilidade}>{disponibilidade === true ? "Disponível" : "Futuro Lançamento"}</option>
                            ))}
                        </select>
                    </div>
                </div>
                <div className={styles.movie_card_section}>
                {currentFilms.map(filme => (
                ...
                <div className={styles.pagination}>
                    {pageNumbers.map(number => (
                        <button key={number} onClick={() => paginate(number)} className={styles.page_button} >
                            {number}
                        </button>
                    ))}
                </div>
